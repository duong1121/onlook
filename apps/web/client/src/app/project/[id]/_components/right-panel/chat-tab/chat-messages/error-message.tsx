...
export const ErrorMessage = observer(() => {
    ...
    const rateLimited: UsageCheckResult | null = true
        ? null
        : {
              reason: 'daily',
    ...
    if (rateLimited) {
        ...
        return (
            <div
                className="flex w-full flex-col items-center justify-center gap-2 text-small px-4 pb-4"
                data-oid="fclv-7o"
            >
                <p
                    className="text-foreground-secondary text-mini my-1 text-blue-300 select-none"
                    data-oid="3uq0rcu"
                >
    ...
    if (errorMessage) {
        return (
            <div
                className="flex w-full flex-row items-center justify-center gap-2 p-2 text-small text-red"
                data-oid="om4l6rl"
            >
                <Icons.ExclamationTriangle className="w-6" data-oid="rlys1.j" />
                <p className="w-5/6 text-wrap overflow-auto" data-oid="43xvdia">
                    {errorMessage}
                </p>
            </div>
        );
